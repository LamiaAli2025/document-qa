...
# Load custom model once (cached for performance)
#@st.cache_resource

# UI Configuration
st.set_page_config(page_title="Arabic Handwriting Extraction", page_icon="✍️")
st.title("✍️ Arabic Handwriting Extraction")

# Model selection
model_choice = st.radio("Select recognition model:", 
                        ["Google Gemini Vision", "Custom Arabic Handwriting Model"])
...
if uploaded_file:
    ...
    st.image(pil_image, caption="Uploaded Image", use_column_width=None)
    
    if st.button("Extract Text"):
        if model_choice == "Google Gemini Vision":
            # Gemini processing
        ...
        elif model_choice == "Custom Arabic Handwriting Model":
            try:
                # Load custom model
                ...
                with st.spinner("Extracting with custom model..."):
                    
                    # Initialize the system
                    ocr_system = OCR_RAG_System(knowledge_base_path='knowledge_base.pkl')
                    # Run prediction
                    response = ocr_system.process_image_with_rag(pil_image, True)
                    # Save the knowledge base for future use
                    ocr_system.save_knowledge_base()
                                       
                    # Display results
                    st.subheader("Extracted Text (Custom Model)")
                    st.write(response)

                    
            except Exception as e:  # Fixed syntax here
                st.error(f"Custom Model Error: {str(e)}")  # Proper error display